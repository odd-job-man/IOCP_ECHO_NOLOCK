...
void PROFILER::ProfileDataOutText(const char* szFileName)
{
	...
	for (int i = 1; DESC_ARR[i] != nullptr; ++i)
	{
		...
		for (int j = 0; j < pPSD->validSampleNumber_; ++j)
		{
			...
			if (pPSD->pProfileSampleArr_[j].iMin[0] == UINT_MAX)
			{
				continue;
			}
			...
			fprintf(pFile, "| %-100s | %-15lld | %-15lld | %-15f | %-15lld | %-15lld | %-10lld |\n",
				pPSD->pProfileSampleArr_[j].szName, totalTime, sum, _average, pPSD->pProfileSampleArr_[j].iMin[0], pPSD->pProfileSampleArr_[j].iMax[0], pPSD->pProfileSampleArr_[j].callNum);
